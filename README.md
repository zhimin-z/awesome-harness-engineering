...
- [HEAAL — A language where grammar itself is the harness](https://github.com/hyun06000/AIL) —  HEAAL (Harness Engineering As A Language) embeds safety constraints directly into grammar, not as afterthoughts. Unlike teams wrapping Python with external harnesses (AGENTS.md files, pre-commit hooks, retry wrappers, output validators), HEAAL puts the harness inside the language.  AIL (AI-Intent Language) is HEAAL's reference implementation. Concretely:  1. **Infinite loops impossible** — no `while` keyword exists. Grammatically impossible. 2. **Fallible operations return Result types** — `http.get`, `to_number`, `parse_json` cannot hide errors. 3. **`pure fn` statically verified** — parser rejects side effects, guaranteeing purity before runtime. 4. **Only `intent` calls LLMs** — all model invocations explicit, type-checked, auditable. 5. **Only `perform env.read` accesses credentials** — cannot hardcode API keys in source. 6. **`perform human.approve` gates irreversible actions** — users see approval cards before posting to public channels.
